...
class P_SE_BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, norm_layer=None,reduction=16):
        super(P_SE_BasicBlock, self).__init__()
        ...
        self.pse = PSELayer(inplanes,planes,reduction)
...
class P_SE_Bottleneck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, norm_layer=None,reduction=16):
        super(P_SE_Bottleneck, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        self.relu = nn.ReLU(inplace=True)
        self.pse = PSELayer(inplanes,planes * self.expansion, reduction)
        self.downsample = downsample
        self.stride = stride
...
class P_SE_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, norm_layer=None):
        super(P_SE_ResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, P_SE_Bottleneck):
                    nn.init.constant_(m.bn3.weight, 0)
                elif isinstance(m, P_SE_BasicBlock):
...
def p_se_resnet18( **kwargs):
    ...
    """
    model = P_SE_ResNet(P_SE_BasicBlock, [2, 2, 2, 2], **kwargs)
    # if pretrained:
...
def p_se_resnet34( **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = P_SE_ResNet(P_SE_BasicBlock, [3, 4, 6, 3], **kwargs)
    # if pretrained:
...
def p_se_resnet50( **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = P_SE_ResNet(P_SE_Bottleneck, [3, 4, 6, 3], **kwargs)
    # if pretrained:
...
def p_se_resnet101( **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = P_SE_ResNet(P_SE_Bottleneck, [3, 4, 23, 3], **kwargs)
    # if pretrained:
...
def p_se_resnet152( **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = P_SE_ResNet(P_SE_Bottleneck, [3, 8, 36, 3], **kwargs)
    # if pretrained:
